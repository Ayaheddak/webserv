...
void    Server::start(pars &parsing)
{
	...
	while (true)
	{
		...
		timeout.tv_sec = 2;
		timeout.tv_usec = 0;
		maxFds = std::numeric_limits<int>::min();//Initialize to smallest possible int value
		...
		backupRead = readFds;//cuz select is destructive
		backupWrite =  writeFds;
		...
		for (std::list<Client>::iterator it = _clients.begin() ; it != _clients.end(); it++)
		{
			...
			{	
				char buff[1025] = {0};
				int rec = read(it->getClientFd() ,buff, 1024);
				...
				if (rec <= 0)					
				{
					...
					FD_CLR(it->getClientFd(), &backupRead);
					FD_CLR(it->getClientFd(), &backupWrite);
					_clients.erase(it);
					break ;
				}
				else
